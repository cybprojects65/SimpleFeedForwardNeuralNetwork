...
//	double maxcycle = 1000;
	...
	public static double preprocess(double element) {
	...
	public double[] propagate(double[] input) {
		if (input.length == griglia[0].length - 1)
			...
			System.out.println("FF-ANN - Error : number of inputs not valid!");
	...
	private void BProp(double[] input, double[] realvalues) {
		...
		for (int g = 0; g < griglia.length - 1; g++) {
			...
			for (int k = 0; k < Ai[g + 1].length; k++) {
				D[k] = (float) (learningRate * arraydelta[g][k]);
			}
			...
			for (int i = 1; i < griglia[g].length; i++) {
				...
				for (int k = 0; k < Ai[g + 1].length; k++) {
					V[k] = (float) (learningRate * (griglia[g][i].generaOutput(Ai[g][i - 1]) * arraydelta[g][k]));
				}
	...
	public void train(double[][] inputvet, double[][] correctoutputvet) {
		...
		if (griglia[griglia.length - 1].length != correctoutputvet[0].length)
			System.out.println("FF-ANN - Error :  the vector of outputs has not a length equal to the output of the network");
		else {
			en = 2;
			int counter = 0;
			double enprec=2;
			...
			while ((en > soglia) && (counter <= maxcycle)) {
				...
				counter++;
				status = (float)counter*100/(float)maxcycle;
				
				if (counter % 100==0){
//					status = (float) counter*100 / (float) maxcycle;
					if (status<100)
						System.out.println("FF-ANN - Progress:"+String.format("%.2g", status)+" Learning Score: " + en);
				}
			...
			status = 100;
			System.out.println("FF-ANN - Progress:"+status+" Final Learning Score: " + en);
			
			if (counter >= maxcycle)
				System.out.println("FF-ANN - finished because reached max cycles");
			else
				System.out.println("FF-ANN - finished because learned under the threshold");
	...
	public void writeout(double numero, double soglia) {
		if (numero < soglia)
			System.out.println("FF-ANN - Exit : " + 0);
		else
			System.out.println("FF-ANN - Exit : " + 1);
	}
	...
	public static synchronized Neural_Network loadNN(String nomeFile) {
		...
		try {
			...
			ex.printStackTrace();
			System.out.println("FF-ANN - Error in reading the object from file " + nomeFile + " .");
		} finally {
			try {
				stream.close();
			} catch (IOException e) {
	...
	public double getDeNormalValue(double numb, int i) {
		return numb * maxinputs[i] + (1 - numb) * mininputs[i];
	}
